import { PoolKey, MAX_POOL_KEYS_RETURNED, Invariant, FungibleToken } from '@invariant-labs/alph-sdk'
...
export function* fetchTokens(poolsWithPoolKeys: PoolWithPoolKey[]) {
  ...
  const { unknownTokensData, knownTokenBalances } = yield* all({
    ...
    knownTokenBalances: call(getTokenBalances, [...knownTokens], fungibleToken, walletAddress)
  })
...
export function* handleGetTokens(action: PayloadAction<string[]>) {
  ...
  try {
    const tokensData = yield* call(getTokenDataByAddresses, tokens, fungibleToken, walletAddress)
