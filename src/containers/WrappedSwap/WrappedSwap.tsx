...
export const WrappedSwap = ({ initialTokenFrom, initialTokenTo }: Props) => {
  ...
  const [tokenFromPriceData, setTokenFromPriceData] = useState<
      ...
      getCoinGeckoTokenPrice(id)
        ...
        .catch(() =>
          setTokenFromPriceData(
            getMockedTokenPrice(
              tokensList[tokenFrom.toString()].symbol,
              network
            )
          )
        )
  ...
  const [tokenToPriceData, setTokenToPriceData] = useState<
    TokenPriceData | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    if (id.length) {
      ...
      getCoinGeckoTokenPrice(id)
        ...
        .catch(() =>
          setTokenToPriceData(
            getMockedTokenPrice(tokensList[tokenTo.toString()].symbol, network)
          )
        )
  ...
  const initialSlippage =
    localStorage.getItem("INVARIANT_SWAP_SLIPPAGE") ?? DEFAULT_SWAP_SLIPPAGE;
      ...
      onSetPair={(tokenFrom, tokenTo) => {
        ...
        if (tokenFrom !== null) {
          localStorage.setItem(
            `INVARIANT_LAST_TOKEN_FROM_${network}`,
            tokenFrom.toString()
          );
        }
        ...
        if (tokenTo !== null) {
          localStorage.setItem(
            `INVARIANT_LAST_TOKEN_TO_${network}`,
            tokenTo.toString()
          );
        }
        if (
