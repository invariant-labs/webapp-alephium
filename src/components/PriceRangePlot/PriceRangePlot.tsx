...
export const PriceRangePlot: React.FC<IPriceRangePlot> = ({
      ...
      data.forEach((tick, index) => {
        if (
          ...
          (dataAfterOmit.length > 0 &&
            ((tick.x - dataAfterOmit[dataAfterOmit.length - 1].x) /
              (plotMax - plotMin) >=
              minXDist ||
              Math.abs(tick.y - dataAfterOmit[dataAfterOmit.length - 1].y) /
                maxVal >=
                minYChange))
        ) {
  ...
  const currentRange = useMemo(() => {
    if (disabled) {
      ...
      if (
        rangeData[rangeData.length - 1].x <
        Math.min(plotMax, data[data.length - 1].x)
      ) {
        rangeData.push({
    ...
    if (!rangeData.length) {
      ...
      if (
        !newRangeData.length ||
        newRangeData[0].x > Math.max(plotMin, rangeData[0].x)
      ) {
  ...
  const currentLayer: Layer = ({ innerWidth, innerHeight }) => {
    ...
    return (
      <svg
        x={(midPrice.x - plotMin) * unitLen - 20}
        y={-20}
        width={40}
  ...
  const brushLayer = Brush(
    ...
    (position) => {
      ...
      onChangeRange?.(
        isXtoY
          ? BigInt(
              Math.min(Number(rightRange.index - tickSpacing), Number(nearest))
            )
          : BigInt(
              Math.max(Number(rightRange.index + tickSpacing), Number(nearest))
            ),
        rightRange.index
      );
    },
    (position) => {
      ...
      onChangeRange?.(
        ...
        isXtoY
          ? BigInt(
              Math.max(Number(leftRange.index + tickSpacing), Number(nearest))
            )
          : BigInt(
              Math.min(Number(leftRange.index - tickSpacing), Number(nearest))
            )
      );
    },
    plotMin,
    ...
    >
      ...
      <ResponsiveLine
        ...
        data={[
          {
            id: "less than range",
            data: currentLessThanRange.length
              ? currentLessThanRange
              : [{ x: plotMin, y: 0 }],
          },
          {
            id: "range",
