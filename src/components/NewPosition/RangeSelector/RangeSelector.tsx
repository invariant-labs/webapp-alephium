...
import { getMaxTick, getMinTick } from "@invariant-labs/alph-sdk";
import {
  Button,
  Checkbox,
  FormControlLabel,
...
export const RangeSelector: React.FC<IRangeSelector> = ({
  ...
  const [cachedConcentrationArray, setCachedConcentrationArray] =
    useState(concentrationArray);
  ...
  const changeRangeHandler = (left: bigint, right: bigint) => {
    ...
    setLeftInputValues(
      calcPriceByTickIndex(leftInRange, isXtoY, xDecimal, yDecimal).toString()
    );
    setRightInputValues(
      calcPriceByTickIndex(rightInRange, isXtoY, xDecimal, yDecimal).toString()
    );
  ...
  const resetPlot = () => {
    if (positionOpeningMethod === "range") {
      ...
      const lowerTick = BigInt(
        Math.min(
          Number(getMaxTick(tickSpacing)),
          Number(midPrice.index) + Number(tickSpacing) * 10
        )
      );
      changeRangeHandler(
        isXtoY ? higherTick : lowerTick,
        isXtoY ? lowerTick : higherTick
      );
  ...
  const autoZoomHandler = (
    ...
    const rightX = calcPriceByTickIndex(
      rightInRange,
      isXtoY,
      xDecimal,
      yDecimal
    ...
    const higherLeftIndex = BigInt(
      Math.max(
        Number(getMinTick(tickSpacing)),
        Number(leftInRange - tickSpacing * 15n)
      )
    );
    ...
    const lowerRightIndex = BigInt(
      Math.min(
        Number(getMaxTick(tickSpacing)),
        Number(rightInRange + tickSpacing * 15n)
      )
    );
          ...
          <Grid>
            ...
            {poolKey !== "" && (
              <Typography className={classes.currentPrice}>
                {formatNumber(midPrice.x, false, 4)} {tokenBSymbol} per{" "}
                {tokenASymbol}
              </Typography>
            )}
          </Grid>
          <Grid
            className={classes.activeLiquidityContainer}
            container
            direction="column"
          >
            <Tooltip
                ...
                <>
                  ...
                  >
                    <Typography className={classes.liquidityDesc}>
                      ...
                      lines in the liquidity chart. Active liquidity is
                      determined by the maximum price range resulting from the
                      statistical volume of exchanges for the last 7 days.
                    </Typography>
                    <img
      ...
      {blocked && (
        <Grid className={classes.blocker}>
          {blockerInfo === "Loading pool info..." ? (
            ...
            <Typography className={classes.blockedInfo}>
              {blockerInfo}
            </Typography>
          )}
        </Grid>
