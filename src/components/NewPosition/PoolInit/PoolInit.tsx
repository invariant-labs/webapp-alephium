...
export const PoolInit: React.FC<IPoolInit> = ({
  ...
  const changeRangeHandler = (left: bigint, right: bigint) => {
    ...
    setLeftInputValues(
      calcPriceByTickIndex(left, isXtoY, xDecimal, yDecimal).toString()
    );
    setRightInputValues(
      calcPriceByTickIndex(right, isXtoY, xDecimal, yDecimal).toString()
    );
  ...
  const resetRange = () => {
    if (positionOpeningMethod === "range") {
      const higherTick = BigInt(
        Math.max(
          Number(minTick),
          Number(midPriceIndex) - Number(tickSpacing) * 10
        )
      );
  ...
  const validateMidPriceInput = (midPriceInput: string) => {
    if (positionOpeningMethod === "concentration") {
      ...
      const validatedPrice =
        validatedMidPrice < MINIMAL_POOL_INIT_PRICE
          ? MINIMAL_POOL_INIT_PRICE
          : validatedMidPrice;
      ...
      const minimalAllowedInput =
        minPriceFromTick < MINIMAL_POOL_INIT_PRICE
          ? MINIMAL_POOL_INIT_PRICE
          : minPriceFromTick;
  ...
  useEffect(() => {
    if (currentPairReversed !== null) {
      const validatedMidPrice = validateMidPriceInput(
        (1 / +midPriceInput).toString()
      );
      ...
      <Grid className={classes.topInnerWrapper}>
        ...
        <Grid className={classes.infoWrapper}>
          <Typography className={classes.info}>
            This pool does not exist yet. To create it, select the fee tier,
            initial price, and enter the amount of tokens. The estimated cost of
            creating a pool is 0.003 ALPH.
          </Typography>
        </Grid>
