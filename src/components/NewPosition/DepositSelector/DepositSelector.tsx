...
export const DepositSelector: React.FC<IDepositSelector> = ({
  ...
  const [hideUnknownTokens, setHideUnknownTokens] = useState<boolean>(
    initialHideUnknownTokensValue
  );
  ...
  useEffect(() => {
    if (
      ...
      ALL_FEE_TIERS_DATA.length !== 0
    ) {
      const tokenAFromPath =
        tokens[tickerToAddress(network, initialTokenFrom)]?.assetAddress ||
        null;
      const tokenBFromPath =
        tokens[tickerToAddress(network, initialTokenTo)]?.assetAddress || null;
  ...
  const getButtonMessage = useCallback(() => {
    ...
    if (
      ...
      return concentrationArray[minimumSliderIndex]
        ? `Set concentration to at least ${concentrationArray[
            minimumSliderIndex
          ].toFixed(0)}x`
        : "Set higher fee tier";
    }
    ...
    if (alphBalance < POOL_SAFE_TRANSACTION_FEE) {
  ...
  useEffect(() => {
    if (tokenA !== null) {
      if (
        getScaleFromString(tokenAInputState.value) >
        Number(tokens[tokenA].decimals)
      ) {
    ...
    if (tokenB !== null) {
      if (
        getScaleFromString(tokenBInputState.value) >
  ...
  return (
    ...
    >
      ...
      <Grid container className={classes.sectionWrapper}>
        <DepositAmountInput
          ...
          onMaxClick={() => {
            ...
            tokenAInputState.setValue(
              printBigint(tokens[tokenA].balance, tokens[tokenA].decimals)
            );
          }}
          balanceValue={
            tokenA !== null
        ...
        <DepositAmountInput
          ...
          onMaxClick={() => {
            ...
            tokenBInputState.setValue(
              printBigint(tokens[tokenB].balance, tokens[tokenB].decimals)
            );
          }}
          balanceValue={
            tokenB !== null
      ...
      {walletStatus !== Status.Initialized ? (
        ...
        />
      ) : getButtonMessage() === "Insufficient ALPH" ? (
        <TooltipHover
