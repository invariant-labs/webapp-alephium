...
export const PositionItem: React.FC<IPositionItem> = ({
  ...
  const [xToY, setXToY] = useState<boolean>(
    initialXtoY(
      tickerToAddress(network, tokenXName),
      tickerToAddress(network, tokenYName)
    )
  );

  const getPercentageRatio = () => {
    const firstTokenPercentage =
      ((tokenXLiq * currentPrice) / (tokenYLiq + tokenXLiq * currentPrice)) *
      100;

    const tokenXPercentageFloat = xToY
      ? firstTokenPercentage
      : 100 - firstTokenPercentage;
        ...
        title={
          isActive ? (
            <>
              ...
              as long as the current price is <b>within</b> the position's price
              range.
            </>
          ) : (
            <>
              The position is <b>inactive</b> and <b>not earning a fee</b> as
              long as the current price is <b>outside</b> the position's price
              range.
            </>
          )
        }
      ...
      >
        <Grid
          ...
          className={classNames(
            classes.fee,
            isActive ? classes.activeFee : undefined
          )}
  ...
  const valueFragment = useMemo(
    () => (
      <Grid
        ...
        wrap="nowrap"
      >
        <Typography className={classNames(classes.infoText, classes.label)}>
          Value
        </Typography>
        <Grid
          className={classes.infoCenter}
          container
          item
          justifyContent="center"
        >
          <Typography className={classes.greenText}>
            {formatNumber(xToY ? valueX : valueY)}{" "}
    ...
    >
      ...
      <Grid container item className={classes.mdInfo} direction="row">
        ...
        >
          <>
            <Typography
              className={classNames(classes.greenText, classes.label)}
            >
              MIN - MAX
            </Typography>
            <Grid
            ...
            >
              {isFullRange ? (
                ...
                <Typography className={classes.infoText}>
                  {formatNumber(xToY ? min : 1 / max)} -{" "}
                  {formatNumber(xToY ? max : 1 / min)}{" "}
                  {xToY ? tokenYName : tokenXName} per{" "}
                  {xToY ? tokenXName : tokenYName}
                </Typography>
              )}
            </Grid>
